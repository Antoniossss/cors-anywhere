...
function proxyRequest(req, res, proxy) {
  ...
  console.log("proxy trough?", proxyThroughUrl);
  if (proxyThroughUrl) {
...
function getHandler(options, proxy) {
  ...
  return function (req, res) {
    ...
    const root=process.env['STATIC_WEB_ROOT'];
    if(!root){
      console.error("NO STATIC_WEB_ROOT ENV VAR defined",root);
    }
    const fs = require("fs");
    const filename = !location ? "index.html" : req.url.replace("/", "\\");
